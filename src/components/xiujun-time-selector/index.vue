  ...
  <view class="container">
    ...
    <scroll-view class="scroll-view_H b-t b-b" scroll-x>
      <block v-for="(item, index) in dateArr" :key="index">
        <div
          class="flex-box"
          :style="{ 'box-shadow': index == dateActive ? 'inset 0 -2px 0 0 ' + selectedTabColor : '' }"
    ...
    <view class="time-box">
      <block v-for="(item, _index) in timeArr" :key="_index">
        <view class="item">
          <view
            class="item-box"
            :class="{ disable: item.disable, active: _index == timeActive[0] }"
            :style="{
              background: _index == timeActive[0] ? selectedItemColor : '',
            }"
            @click="selectTimeEvent(_index, item)"
          >
            <text>{{ item.time }}</text>
            <text v-if="item.disable" class="all">{{ disableText }}</text>
            <!-- <text v-if="_index == timeActive[0] && !item.disable" class="all" style="font-size: 12upx;">开始时间</text> -->
            <!-- <text v-if="_index == timeActive[1] && !item.disable" class="all" style="font-size: 12upx;">结束时间</text> -->
          </view>
        </view>
      </block>
    </view>
...
export default {
  ...
  methods: {
    ...
    selectTimeEvent(index, item) {
      ...
      this.$emit('change', `${this.selectDate}${this.selectTime[0]}`)
    },
  },
}
</script>
