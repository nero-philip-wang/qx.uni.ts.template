<template>
  <view class="app">
    <view class="nav-wrap">
      <u-sticky>
        <u-tabs :list="navs" :counts="navCounts" :current="navCurrent" @change="onNavBarChange"></u-tabs>
        <!-- <mix-nav-bar :navs="navs" :counts="navCounts" :current="navCurrent" @onChange="onNavBarChange"></mix-nav-bar> -->
      </u-sticky>
    </view>

    <MescrollBody ref="mescrollRef" :top="0" :up="upOption" @init="mescrollInit" @down="downCallback" @up="loadData">
      <view v-for="(item, index) in list" :key="index" class="item column" @click="$goto('/pages/order/detail?id=' + item.id)">
        <view class="i-header row b-b">
          <text class="time fill">{{ item.creationTime | timef }}</text>
          <text class="state" :class="{ gray: item.status === 10 }">{{ item.status_text }}</text>
        </view>
        <!-- 单商品 -->
        <view v-if="item.items.length === 1" class="goods-wrap row">
          <view class="image-wrapper lazyload lazypic" :class="{ loaded: item.loaded }">
            <image :src="item.items[0].cover" mode="aspectFill" lazy-load="true"></image>
          </view>
          <view class="right column">
            <text class="title clamp">{{ item.items[0].title }}</text>
            <text class="attr">{{ '-' }}</text>
            <view class="r-b">
              <text class="price">￥{{ item.items[0].price | yuan }}</text>
              <text class="number">x{{ item.items[0].quantity }}</text>
            </view>
          </view>
        </view>
        <!-- 多商品 -->
        <scroll-view v-else class="scroll-view" scroll-x>
          <view class="goods-list">
            <view
              v-for="(gItem, gIndex) in item.items"
              :key="gIndex"
              class="image-wrapper lazyload lazypic"
              :class="{ loaded: item.loaded }"
            >
              <image :src="gItem.cover" mode="aspectFill" lazy-load="true"></image>
            </view>
          </view>
        </scroll-view>
        <view class="price-wrap">
          <text>共{{ orderItemCount(item) }}件 实付:</text>
          <text class="price">￥{{ item.orderAmount }}</text>
        </view>
        <view class="btn-group row" @click.stop.prevent="stopPrevent">
          <view
            v-if="item.status === 4 || item.status === 10 || item.status === 11 || item.status === 14"
...
export default {
  ...
  methods: {
    ...
    async loadData(e) {
      ...
      //   }
      const curList = await search(sendData) /* await this.$request('order', 'getList', sendData) */
      if (e.num === 1) {
        // 第一页清空数据重载
        this.list = []
        this.loaded &&
