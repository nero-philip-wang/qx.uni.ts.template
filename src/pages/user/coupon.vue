<template>
  <view class="app">
    <view class="nav-wrap">
      <u-tabs :list="navs" :counts="navCounts" :current="navCurrent" :is-scroll="false" @change="onNavBarChange"></u-tabs>
    </view>
    <MescrollBody ref="mescrollRef" :top="84" :up="upOption" @init="mescrollInit" @down="downCallback" @up="loadData">
      <view v-for="(item, index) in list" :key="index" class="coupon-item" :class="{ disabled: item.state !== 1 }">
        <view class="con">
          <view class="left">
...
export default {
  ...
  methods: {
    async loadData(e) {
      ...
      const res = { data: [] } // await this.$request('coupon', 'getUserCouponList', sendData);
