<template>
  <view class="app" :class="{ overhidden: list.length === 0 }">
    ...
    <!-- 产品列表 -->
    <MescrollBody ref="mescrollRef" :top="headerHeight" :up="upOption" @init="mescrollInit" @down="downCallback" @up="loadList">
      <product-list ref="productList" :list="list" :list-type="listType"></product-list>
    </MescrollBody>
  ...
  methods: {
    ...
    calcCateRect() {
      ...
      Promise.all(tasks).then(() => {
        const cur = this.cateList.filter((item) => item._id === this.curCateItem._id)
        if (cur.length > 0) {
