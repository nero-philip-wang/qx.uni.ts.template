<template>
  <view class="app">
    ...
    <!-- 轮播图 -->
    <mix-swiper :list="data.covers"></mix-swiper>
    <view class="introduce column">
      <text class="title">{{ data.title }}</text>
      <view class="price-wrap row">
        <mix-price-view :price="data.minRetailPrice | yuan" :size="40"></mix-price-view>
        <text v-if="data.markingPrice > data.minRetailPrice" class="m-price">￥{{ data.markingPrice | yuan }}</text>
        <text v-if="loaded && !data.freightTemplateId" class="tag">免邮费</text>
      </view>
      <view class="bot row">
        <text class="fill">销量: {{ data.sales || '-' }}</text>
        <text class="fill">库存: {{ data.stock || '-' }}</text>
        <text class="fill">浏览量: {{ data.look_num || '-' }}</text>
      </view>
    </view>
    <!--  分享 -->
    ...
    <view class="c-list">
      ...
      <view class="row b-b">
        ...
        <view class="con">
          <text class="service"> 不满意免费更换 </text>
        </view>
      </view>
    </view>
    <!-- 评价 -->
    ...
    <view class="detail-desc">
      ...
      <image v-for="item in data.covers" :key="item" :src="item" mode="aspectFill" style="width:100%;height:500px" />

      <!-- <rich-text :nodes="data.content"></rich-text> -->
    </view>
    <!-- 底部操作菜单 -->
    <bottom-operation :info-data="data" @onOprationClick="onOprationClick"></bottom-operation>
    <!-- 规格面板 -->
    <sku v-if="data.id" ref="skuPopup" :info-data="data" @setSku="setCurrentSku" @addToCart="addToCart" @buyNow="buyNow"></sku>
    <!-- loading -->
...
import mixPriceView from './components/mix-price-view' // sku面板
  ...
  methods: {
    ...
    getConfirmData() {
      setItems([
        {
          ...this.data.skus[0],
          quantity: this.$refs.skuPopup.buyNumber || 1,
