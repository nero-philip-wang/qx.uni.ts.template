<template>
  <u-popup ref="popup" v-model="isshow" type="bottom" mode="bottom">
    <view class="content column">
      <text class="mix-icon icon-guanbi" @click="close"></text>
      <view class="pro-info row">
        <image class="pic" :src="currentSku.image || infoData.covers[0]" mode="aspectFill"></image>
        <view class="right column">
          <text class="price">￥{{ currentSku.price || infoData.minRetailPrice | yuan }}</text>
          <text class="stock">库存：{{ currentSku.stock || infoData.stock || '-' }}</text>
      ...
      <scroll-view class="c-wrap" scroll-y="true">
        ...
        <view class="number-wrap row">
          ...
          <u-number-box v-model="buyNumber" :min="1" :max="currentSku.stock || 999" @change="onNumberChange"></u-number-box>
        </view>
      </scroll-view>
...
<script>
import { getUnique } from '@/utils/cy.js'
export default {
  props: {
    infoData: {
      type: Object,
  ...
  created() {
    ...
    this.specList = specList.map((item) => {
      item.attrList = attrList
        .filter((attr) => attr.parent === item.name)
        .map((attr, index) => {
          return {
            checked: index === 0,
            ...attr,
