<template>
  <view class="mix-botoom-operation row">
    <view class="nav-group row">
      ...
      <button open-type="contact" show-message-card class="nav column center" :class="{ active: is_fav === 1 }" @click="changeFav">
        <text class="mix-icon" :class="is_fav === 1 ? 'icon-shoucang' : 'icon-shoucang-1'"></text>
        <text class="tit">客服</text>
      </button>
    </view>
    <view class="btn-group row">
      ...
      <view class="btn center single" @click="onOprationClick('buy')">
        <text>立即购买</text>
      </view>
    </view>
